...
chemin_fichier_log = os.path.join(os.path.dirname(__file__), 'encrypted_credentials.dat')
...
def check_model_performance():
    try:
        ...
        if most_recent_mse > THRESHOLD:
            ...
            largest_gap = max([(b - a) for a, b in zip(above_threshold_indices[:-1], above_threshold_indices[1:])])
            center_idx = above_threshold_indices[above_threshold_indices.index(largest_gap // 2)]
...
def send_email(graph_path):
    recipients = ['thomascouloud@hotmail.fr', 'sebastien.moitel@gmail.com', 'richard.bonfils.data@gmail.com']
    ...
    with open(graph_path, 'rb') as attachment:
        ...
        part.add_header('Content-Disposition', 'attachment; filename=performance_graph.png')
        msg.attach(part)
