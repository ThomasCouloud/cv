...
def train_and_save_model(train_X, train_y, best_params):
    xgb_grid = GridSearchCV(
        xgb.XGBRegressor(), best_params, cv=5, n_jobs=5, verbose=True
    )
    ...
    timestamped_model_filename = os.path.join(
        path_to_model_directory, f"model_{current_datetime}.joblib"
    )

    joblib.dump(best_model, timestamped_model_filename)

    return {
        "message": f"Model trained and saved successfully to {timestamped_model_filename}",
